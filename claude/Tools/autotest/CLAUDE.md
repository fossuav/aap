...
Denying a sensor by parameter is not the same as removing it. `RNGFND1_MIN` above `RNGFND1_MAX` leaves no distance the backend can call Good, which reads like a dead sensor - but every reading is then `OutOfRangeLow`, and that is still data to any code that substitutes a known ground clearance for a short reading. A leg written to assert "this flight measured no terrain offset" could not tell that from "this flight measured its own ground clearance", failed on a tree carrying that substitution, and was read as a bug in the code under test for a day. Deny the driver instead: pointing the range finder away from `ROTATION_PITCH_270` makes `readRangeFinder()` skip it whatever the backend reports. Before writing a test around a disabled sensor, check what the consumer does with the status you are actually producing.
