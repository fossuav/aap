...
```bash
python3 .claude/skills/pr-review/pr_review.py evidence show
```
...
- The bot's findings are a second opinion, like Codex's in step 5, not orders. It reproduces most of its numbers, but a finding still gets checked against the source before it drives an edit.
...
|---------------------|------------------------|
| `libraries/AP_NavEKF3/`, `libraries/AP_NavEKF/`, `libraries/AP_AHRS/`, `libraries/AP_DAL/` | `libraries/AP_NavEKF3/CLAUDE.md` - state vector, DAL and Replay rules, bias inhibition, yaw source handling, analysis method |
| `libraries/AP_HAL_ChibiOS/hwdef/`, `Tools/AP_Bootloader/`, `Tools/bootloaders/` | `libraries/AP_HAL_ChibiOS/hwdef/CLAUDE.md`, applied by `/hwdef-check` |
| `Tools/autotest/` | `Tools/autotest/CLAUDE.md` - event waits, registration, speedup, harness gotchas |
| `libraries/AP_Scripting/`, any `.lua` | `libraries/AP_Scripting/CLAUDE.md` (plus `CLAUDE_CRSF_MENU.md` and `CLAUDE_VEHICLE_CONTROL.md` when the script touches CRSF menus or vehicle control) |
| `ArduPlane/`, QuadPlane, TECS | `ArduPlane/CLAUDE.md` |
...
the description for each where a maintainer needs it, and the argument and
the number under its detail heading. A
