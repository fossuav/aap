...
- **Review it before you open it:** run `/pr-review` on the branch before creating the PR, and again whenever review feedback arrives. It runs the mechanical CI gates, reviews the diff against the rules in this playbook, cross-checks the findings with an independent Codex pass, and drives the fixes. Clearing the gate first spends a maintainer's attention on the design rather than on whitespace and commit prefixes.
