...
```bash
python3 .codex/skills/log-analyze/flow_cal_check.py <log.bin> [--speed-min 1.0] [--qual-min 50] [--height auto|aglkf|rfnd]
```

The **scale** check needs a flight with **GPS** (outdoors, as truth - flow may or may not be the nav source), flow logged (`OF` or `ROFH`), `ATT`, and a height source (`RFND` or the AGL-KF `XKF6`). It compares the gyro-compensated flow rate to the flow rate implied by GPS velocity, per body axis.

The **sensor-rate check** needs only `IMU` and the flow message, so it runs first and is valid on a GPS-less indoor bring-up flight - which makes it the one check you can get from the first hover on a new airframe. Between them they report:

- **Sensor-rate check (run this conclusion first).** It regresses the flow node's reported body rate against the flight-controller IMU gyro during rotation. `slope ~1.0` means the sensor clocks its output at the right rate. It doubles as an orientation check: the FC IMU is in the body frame and `FLOW_ORIENT_YAW` is applied to the node's flow *and* gyro alike, so `+1.0` on both axes confirms `FLOW_ORIENT_YAW` - a 180 deg error reads `-1.0` on both, a 90 deg error swaps the axes and collapses the correlation. A slope far from 1 (e.g. ~0.5) means the **whole sensor** - flow AND its own gyro - is mis-rated, typically a DroneCAN flow node (ARK Flow/HereFlow) reporting `integration_interval` wrong or a bad `IMU_INTEG_RATE`/firmware. A `FLOW_*SCALER` **cannot** fix that (it scales `flowRate`, not `bodyRate`), so when the slope is off the helper suppresses the scaler suggestion. The fix for a HereFlow/DroneCAN node is **`FLOW_HF_RATEF = 1/slope`** (e.g. slope 0.52 -> `FLOW_HF_RATEF=1.92`): it scales flow and gyro together via `integralToRate`, so the EKF's gyro compensation stays valid - the thing the scaler can't do. The root cause is still sensor-side (the node's `integration_interval` / `IMU_INTEG_RATE` / firmware), so `FLOW_HF_RATEF` is a flight-controller-side correction, not a substitute for fixing the sensor. (`FLOW_HF_RATEF` is a SmallFastDrone-fork parameter; on stock ArduPilot the only fix is the sensor firmware.) Validated on an ARK Flow vehicle: slope 0.52 -> 1.00 and per-axis `flow/ideal` 0.50 -> 1.0 after setting it.
...
`--height` picks the range source, and **the default is raw `RFND.Dist`** - the honest reference. `aglkf` uses the AGL-KF height (`XKF6.HAgl`) that the EKF actually feeds the flow fusion when `EK3_OPTIONS` bit 4 is set.

It is tempting to calibrate against `aglkf` on the grounds that matching the height the EKF uses makes the EKF's flow velocity correct by construction. **Do not** - the AGL KF is a filtered product, and any lag or attenuation in it lands directly in the fitted scale. On an ARK Flow octaquad the same flight read `flow/ideal` 0.55 with `corr` 0.87 against `XKF6.HAgl` and 0.97 with `corr` 0.97 against `RFND.Dist`; the AGL KF lagged the rangefinder by 0.7 s, and calibrating against it would have baked a bogus +650 scaler into a sensor that was already correct. Calibrate the sensor against the rangefinder, validate the rangefinder against GPS Doppler (below), and fix the AGL KF as a separate problem. Use `--height aglkf` only to measure how much the AGL KF is distorting things - a big gap between the two runs is a bug report about the AGL KF, not a calibration result.

`--gps-lag` scans for the flow/GPS time alignment and reports what it picked. Treat it as diagnostic, not truth: on one airframe it preferred 0.35-0.45 s while an independent check - raw GPS velocity against the EKF's delay-compensated velocity - put the real lag near 0.10 s. If the fitted scale moves a lot with the lag, the fit is weak; if it barely moves, the alignment is right.
...
- **Hovering near the rangefinder's max range truncation-biases the scale downward.** Samples above the range cap are dropped, so only the short readings survive, the height used comes out low, the GPS-implied flow comes out high, and `flow/ideal` comes out low. On an octaquad calibrating at 8-9 m against a cap of 8 m this read 0.75 on an axis that was actually 1.03 - and it read "reliable" with `corr` 0.96, because truncation biases the fit without scattering it. The helper now caps at `RNGFND1_MAX` and warns when more than 20% of samples sit within 10% of it; a degenerate height spread (p5 close to the mean) is the fingerprint. Calibrate well inside the sensor's range.
- **The sign convention is not obvious and the helper used to get it wrong.** `writeOptFlowMeas` negates the raw flow before compensating (`AP_NavEKF3_Measurements.cpp`, `flowRadXYcomp = -rawFlow + bodyRate`) while `ROFH`/`OF` log the driver's pre-negation values. Comparing the logged fields straight against the EKF's `losPred` model reports "sign inverted" on every log regardless of `FLOW_ORIENT_YAW`. The helper negates for you; a residual negative slope now means a real orientation problem.
...
```bash
python3 .codex/skills/log-analyze/log_extract.py overview <logfile>
```
...
python3 .codex/skills/log-analyze/log_extract.py overview <logfile.tlog> --system 5
```
...
```bash
# EKF data for core 0
python3 .codex/skills/log-analyze/log_extract.py extract <logfile> \
...
python3 .codex/skills/log-analyze/log_extract.py stats <logfile> \
...
python3 .codex/skills/log-analyze/log_extract.py compare <logfile> --recipe altitude

# Attitude: actual vs desired roll/pitch/yaw
python3 .codex/skills/log-analyze/log_extract.py compare <logfile> --recipe attitude

# Vibration: raw accel + VIBE values
python3 .codex/skills/log-analyze/log_extract.py compare <logfile> --recipe vibration

# EKF variances and health
python3 .codex/skills/log-analyze/log_extract.py compare <logfile> --recipe ekf_health

# RC input vs output
python3 .codex/skills/log-analyze/log_extract.py compare <logfile> --recipe rc

# PID components per axis
python3 .codex/skills/log-analyze/log_extract.py compare <logfile> --recipe pid_yaw
python3 .codex/skills/log-analyze/log_extract.py compare <logfile> --recipe pid_roll
python3 .codex/skills/log-analyze/log_extract.py compare <logfile> --recipe pid_pitch

# Rate tracking per axis (desired vs actual vs output)
python3 .codex/skills/log-analyze/log_extract.py compare <logfile> --recipe rate_yaw
python3 .codex/skills/log-analyze/log_extract.py compare <logfile> --recipe rate_roll
python3 .codex/skills/log-analyze/log_extract.py compare <logfile> --recipe rate_pitch
python3 .codex/skills/log-analyze/log_extract.py compare <logfile> --recipe rate_all

# Motor outputs
python3 .codex/skills/log-analyze/log_extract.py compare <logfile> --recipe motor_output

# Controller RMS values (oscillation indicator)
python3 .codex/skills/log-analyze/log_extract.py compare <logfile> --recipe ctrl_rms

# Custom comparison (prefix with - to negate)
python3 .codex/skills/log-analyze/log_extract.py compare <logfile> \
...
```bash
# Plot a recipe
python3 .codex/skills/log-analyze/log_extract.py plot <logfile> \
...
for log in log1.bin log2.bin ... ; do
    for inst in 0 1 2 3; do
        python3 .codex/skills/log-analyze/log_extract.py stats "$log" \
...
```bash
python3 .codex/skills/log-analyze/log_extract.py plot <log> \
...
| "Source-set / failover recommendation" | See EKF source-set playbook note in `libraries/AP_NavEKF3/AGENTS.override.md` — source sets are manually switched, not automatic failover. Do not recommend `EK3_SRC2_*` as a "fallback" for glitch response. |
...
python3 .codex/skills/log-analyze/log_extract.py extract <tlog> --types VFR_HUD \
    --fields alt,climb,airspeed,groundspeed,heading --system 5

# Position (lat/lon/alt)
python3 .codex/skills/log-analyze/log_extract.py extract <tlog> --types GLOBAL_POSITION_INT \
    --fields lat,lon,alt,relative_alt,vx,vy,vz --system 5

# GPS fix status
python3 .codex/skills/log-analyze/log_extract.py extract <tlog> --types GPS_RAW_INT \
    --fields fix_type,lat,lon,alt,satellites_visible --system 5

# EKF health
python3 .codex/skills/log-analyze/log_extract.py extract <tlog> --types EKF_STATUS_REPORT \
    --fields flags,velocity_variance,pos_horiz_variance,pos_vert_variance,compass_variance --system 5

# Nav controller
python3 .codex/skills/log-analyze/log_extract.py extract <tlog> --types NAV_CONTROLLER_OUTPUT \
    --fields nav_roll,nav_pitch,alt_error,aspd_error,wp_dist --system 5

# Attitude
python3 .codex/skills/log-analyze/log_extract.py extract <tlog> --types ATTITUDE \
    --fields roll,pitch,yaw --system 5

# Plot altitude with custom sources
python3 .codex/skills/log-analyze/log_extract.py plot <tlog> \
