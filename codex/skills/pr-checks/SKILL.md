...
`.codex/skills/pr-checks/ci_failures.py` works around that by downloading each
...
```bash
# Current branch's PR
python3 .codex/skills/pr-checks/ci_failures.py

# A specific PR (number, URL, or branch). For a fork PR to upstream, name the repo:
python3 .codex/skills/pr-checks/ci_failures.py 33312 --repo ArduPilot/ardupilot
```
...
python3 .codex/skills/pr-checks/ci_failures.py 33312 --repo ArduPilot/ardupilot --raw quadplane
```
