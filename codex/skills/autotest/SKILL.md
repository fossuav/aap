...
output, warns about a held lock, and isolates the run from other clones:

```bash
python3 .codex/skills/autotest/run_autotest.py test.Copter.<Test>
...
prefix also makes the command shape harder to review. If a previous run left a
live lock, clear the wedged process (with the user's ok) - don't work around it
by hand.
...
Use the `autotest_results.py` helper at `.codex/skills/autotest/autotest_results.py` to inspect results — do **not** grep / tail / head per-test files by hand. The helper parses the per-test buildlogs into structured output.

```bash
# Pass/fail summary across all tests in buildlogs
python3 .codex/skills/autotest/autotest_results.py summary

# Filter by vehicle
python3 .codex/skills/autotest/autotest_results.py summary --vehicle ArduCopter

# Show every failing test with reason + exception block
python3 .codex/skills/autotest/autotest_results.py failures

# Full failure context for one test (last N lines + exception)
python3 .codex/skills/autotest/autotest_results.py failure AltHold --lines 150

# List .BIN / .tlog logs produced by the run (feed these into /log-analyze)
python3 .codex/skills/autotest/autotest_results.py logs
```
