...
**Put the new ID at the end of `LOG_IDS_FROM_DAL`, never in the middle.** tridge's rule on PR #34292, given as "to preserve existing replay logs". Keep `LOG_STRUCTURE_FROM_DAL` in the same order as the ID list.

The renumbering is real: measured across two builds of the same tree, moving ROFM out of the middle took it from type 158 to 164 and shifted REPH, RSLL, REVH, RWOH, RBOH and RTER with it. **What it does not appear to break, as of 2026-09-10, is Replay** - so state the rule, not a mechanism for it. Replay dispatches on the four-character name from the log's own `FMT`; `LogReader::handle_log_format_msg` copies each input `FMT` into the output verbatim; `Write_Emit_FMT` suppresses any format Replay would otherwise add outside types 220-230 in a Replay build; and Replay's own output messages sit *before* the DAL block in the enum (`LOG_IDS_FROM_NAVEKF3` at `AP_Logger/LogStructure.h:1288` against `LOG_IDS_FROM_DAL` at `:1341`, so XKF1 is type 44) and do not move. Replaying one log with a mid-list Replay binary and an end-of-list one gave **byte-identical output**.

Follow the rule regardless - it is the maintainer's, it costs nothing, and stable on-disk numbering matters to anything outside this tree that caches ID to name. Just do not repeat the "two names for one type byte" story, which is what this playbook said until it was measured.
